...
const updateOrgRoles = (options, resolve, reject) => {
  try {
    ...
    // only admin or spv can update roles
    const isAuthorized =
      isAdmin(context.user._id, host) || Roles.userIsInRole(context.user._id, 'spv', host);
    if (!isAuthorized) throw new Error('You dont have the authorization to update Org Roles');
